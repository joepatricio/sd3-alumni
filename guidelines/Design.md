# USJ-R SEA Alumni Project Design System

## Overview
This document outlines the core styling conventions, color palettes, and structural layouts governing the USJ-R SEA Alumni Platform.
...
- **USJ-R Green (`#1a5f3f`)**: Kept for secondary elements honoring legacy university branding.
- **USJ-R Green Prominent Hover (`#2d7a4f`)**: Applied when interacting with secondary components.
...
## Business Charts
- **Functional Over Flashy**: Charts are designed for utilitarian data interpretation rather than decorative flair. Use clear, accessible color scales that map logically to the data (e.g., semantic colors for statuses: green for 'Completed', yellow for 'Pending', red for 'Failed').
- **Interactive Legends**: Allow users to isolate data points. Clicking a legend item should grey out or hide other categories, letting administrators focus on specific metrics without visual noise.
- **Actionable Tooltips & Drill-Downs**: Data visualization should always act as a gateway to deeper actions. Where applicable (e.g., Top Donors), pair visualizations with immediate access to profiles or contact actions.
- **Contextual Scales**: Use simple layout components for charts, avoiding overlapping elements. Ensure labels have enough contrast against background colors to maintain readability.
